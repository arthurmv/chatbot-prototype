...
async function getSMS( id ) {
    const sql = "SELECT s.* FROM sms s JOIN conversation c ON s.conversation_id = c.id WHERE s.id = $1 AND c.state IS NOT NULL";
...
async function getWeb( id ) {
    const sql = "SELECT w.* FROM web w JOIN conversation c ON w.conversation_id = c.id WHERE w.id = $1 AND c.state IS NOT NULL";
    return await db.oneOrNone( sql, [ id ] );
}
