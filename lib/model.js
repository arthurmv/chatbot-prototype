...
async function purge() {
    var cutOff = moment().subtract( 1, "hours" );
    const sql = "DELETE FROM $1~ WHERE conversation_id NOT IN ( SELECT id FROM conversation WHERE last_update < $2 )";
    await db.none( sql, [ "web", cutOff ] );
    await db.none( sql, [ "sms", cutOff ] );
}
