...
async function newSMS( req, res ) {
    ...
    await model.vectors.sms.new( req.query.From, id );
    twiml.message( resp );
...
async function receiveSMS( req, res ) {
    var twiml = new twilio.TwimlResponse();
    var sms = await model.vectors.sms.get( req.query.From );
    if( sms === null )
        return await newSMS( req, res );
    var conv = await model.conversation.get( sms.conversation_id );
    var resp = await reply( conv, req.query.Body );
    twiml.message( resp );
    res.end( twiml.toString() );
}
